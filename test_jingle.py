...
async def feeder(audi:ezaud.Audi):
	...
	await s(0.1)
	audi.add(audi.wg.Sin(dur=1,att=0.7,vol=1,freq=ezaud.note("A4b")))		#yes, you have to create a new object for every note.
	await s(1)
	audi.add(audi.wg.Sin(dur=0.1,vol=0.5,freq=ezaud.note("C4")))
	await s(0.1)
	audi.add(audi.wg.Sin(dur=0.3,vol=0.25,freq=ezaud.note("E4")))
	await s(0.3)
	audi.add(audi.wg.Sin(dur=0.1,vol=0.125,freq=ezaud.note("C4")))
	await s(0.1)
	audi.add(audi.wg.Sin(dur=0.3,vol=0.0625,freq=ezaud.note("A4")))
	await s(0.3)
	audi.add(audi.wg.Sin(dur=0.1,vol=0.03125,freq=ezaud.note("C4")))
	await s(0.1)
	audi.add(audi.wg.Sin(dur=0.3,vol=0.015625,freq=ezaud.note("E4")))
	await s(0.3)
	audi.add(audi.wg.Sin(dur=0.1,vol=0.0078125,freq=ezaud.note("C4")))
	await s(0.4)
	...
	await s(0.2)
	audi.add(audi.wg.Sin(dur=0.2,vol=0.3,freq=ezaud.note("A4")))
